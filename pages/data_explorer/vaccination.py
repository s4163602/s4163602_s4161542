...
def get_page_html(form_data):
    ...
    db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "immunisation.db"))
...
<body class="page-body">
  ...
  <!-- ==========================
       FILTER BAR — TABLE 1
       ========================== -->
  <form action="/data_explorer/vaccination#table1" method="GET" class="filter-bar">
"""

    # --- Antigen Dropdown (Table 1) ---
    page_html += "<select name='var_antigen'><option value=''>Antigen</option>"
    ...
    # --- Submit + keep Table 2 state as hidden fields + Excel button ---
    page_html += f"""
    <input type="submit" value="Show" class="btn">

    <!-- keep Table 2's state -->
    ...
    <!-- Excel download for Table 1 -->
    <button type="button" class="download-btn" data-target="vaccination-table-1" title="Download Table 1 as Excel">
      ⬇ Download Excel
    </button>
  </form>
"""
  ...
  <div class="table-container">
    ...
    # ================= SEPARATE SECTION: TABLE 2 =================
    page_html += """
  <hr style="margin:2rem 0; border:none; border-top:1px solid #e5e7eb;">
  <h2 id="table2" class="table-heading">Data Summary by Threshold</h2>

  <form action="/data_explorer/vaccination#table2" method="GET" class="filter-bar">
"""

    # --- Antigen Dropdown (Independent for Table 2) ---
    page_html += "<select name='var_antigen_summary'><option value=''>Antigen</option>"
    for a_id, a_name in pyhtml.get_results_from_query(db_path, "SELECT AntigenID, name FROM Antigen ORDER BY name;"):
    ...
    # --- Threshold Input ---
    page_html += f"""
    <input type="number" name="var_threshold" min="0" max="100" value="{var_threshold}"
           class="threshold-input" title="Coverage Threshold (%)">
    <input type="submit" value="Show Summary" class="btn">

    <!-- keep Table 1's state -->
    ...
    <!-- Excel download for Table 2 -->
    <button type="button" class="download-btn" data-target="vaccination-table-2" title="Download Table 2 as Excel">
      ⬇ Download Excel
    </button>
  </form>
"""
  ...
  <div class="table-container">
    ...
    if var_antigen_summary:
        ...
        query_2 = f"""
        SELECT a.name, v.year, COUNT(c.CountryID) AS num_countries, c.region
        FROM Vaccination v
        JOIN Country c ON v.country = c.CountryID
        JOIN Antigen a ON v.antigen = a.AntigenID
        WHERE v.antigen = '{var_antigen_summary}'
          AND v.coverage >= {threshold_float}
  ...
  <script>
    // Export a table (by element id) to an .xlsx file using SheetJS
    ...
    // Attach listeners to all buttons with data-target
    document.querySelectorAll('.download-btn[data-target]').forEach(function(btn) {{
      btn.addEventListener('click', function() {{
        var targetId = this.getAttribute('data-target');
        // Build a sensible filename using current filters
        var antigen = "{var_antigen or ''}";
        var year = "{var_year or ''}";
        var antigenSummary = "{var_antigen_summary or ''}";
        var threshold = "{var_threshold or ''}";

        var filename = "vaccination_table.xlsx";
        if (targetId === "vaccination-table-1") {{
          filename = ("vaccination_detailed" +
                      (antigen ? "_antigen-" + antigen : "") +
                      (year ? "_year-" + year : "") + ".xlsx");
        }} else if (targetId === "vaccination-table-2") {{
          filename = ("vaccination_summary" +
                      (antigenSummary ? "_antigen-" + antigenSummary : "") +
                      (threshold ? "_ge-" + threshold + "pct" : "") + ".xlsx");
        }}
